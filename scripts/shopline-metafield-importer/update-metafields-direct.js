...
async function createOrUpdateMetafield(productId, metafield) {
    ...
    return { success: false, key: metafield.key, error: text.substring(0, 200) };
}

async function main() {
    console.log('═══════════════════════════════════════════');
    console.log('  批量更新产品元字段');
    console.log('═══════════════════════════════════════════');
    console.log(`\n产品 ID: ${CONFIG.productId}`);
    ...
    for (const mf of metafields) {
        console.log(`处理: ${mf.key}...`);
        const result = await createOrUpdateMetafield(CONFIG.productId, mf);
