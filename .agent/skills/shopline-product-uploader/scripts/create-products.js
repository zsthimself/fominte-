...
const CONFIG = {
  ...
  csvPath: path.join(process.cwd(), 'FM0301000275_v2.csv'),
...
function buildProductData(row) {
  ...
  if (masterImage && (masterImage.startsWith('http://') || masterImage.startsWith('https://'))) {
    productData.images = [
      {
        src: masterImage,
...
/**
 * 将 Markdown 文本转换为 rich_text_field JSON 格式
 * Shopline 的 rich_text_field 需要特定的 JSON 结构
 */
...
/**
 * 提取 CSV 行中的元字段数据
 * 对 rich_text_field 类型自动转换为 JSON 格式
 */
function extractMetafields(row) {
  ...
  for (const [csvColumn, mfConfig] of Object.entries(METAFIELD_MAPPING)) {
    ...
    if (value && value.trim()) {
      // 对 rich_text_field 类型进行 JSON 格式转换
      let finalValue = value.trim();
      if (mfConfig.type === 'rich_text_field') {
        finalValue = markdownToRichText(value.trim());
      }
...
async function processRow(row, existingProductMap) {
  ...
  if (existingProductMap.has(handle)) {
    ...
    if (!createResult.success) {
      ...
      if (
        createResult.error &&
        createResult.error.includes("handle is already taken")
      ) {
        ...
        if (existingProduct) {
          ...
          console.log(
            `   ❌ 创建失败且无法获取现有产品: ${createResult.error}`,
          );
          return {
            success: false,
            reason: "create_failed_and_lookup_failed",
            handle,
...
async function main() {
  ...
  const existingProductMap = new Map(
    existingProducts.map((p) => [p.handle, p.id]),
  );
